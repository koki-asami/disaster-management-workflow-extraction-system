...
from dmwe_core.utils.s3_storage import (
    get_pdf_url,
    upload_chart_code,
    get_chart_code,
    upload_graph_data,
    ...
    try:
        ...
        if chart_id:
            ...
            try:
                ...
                if user_sub:
                    update_expr += ', user_sub = :us'
                    expr_values[':us'] = user_sub
                if title:
                    update_expr += ', title = :t'
                    expr_values[':t'] = title
                if pdf_object_key:
...
def get_default_pdf(location_name, user_sub=None):
        ...
        if 'Items' in response and len(response['Items']) > 0:
            ...
            if user_sub:
                items = [
                    item
                    for item in items
                    if not item.get('user_sub') or item.get('user_sub') == user_sub
                ]
            if not items:
                return None
...
def list_uploads(user_sub: str | None = None) -> list[dict]:
    """List upload records, optionally scoped to the current owner."""
    table = get_uploads_table()
    resp = table.scan()
    items = resp.get("Items", [])
    if user_sub is None:
        return items
    return [item for item in items if item.get("user_sub") == user_sub]
...
def mark_upload_complete(
    ...
    try:
        if user_sub is not None:
            existing = get_upload(upload_id)
            if not existing or existing.get("user_sub") != user_sub:
                return None
...
def delete_upload_record(upload_id: str, user_sub: str | None = None) -> bool:
...
def create_job(
    ...
    user_sub: optional Cognito subject (or API user id) for multi-tenant isolation.
    """
...
def update_job_progress(
    ...
    openai_batch_request_total: int | None = None,
    openai_batch_request_completed: int | None = None,
    openai_batch_request_failed: int | None = None,
    ...
    if batch_id is not None:
        expr_parts.append("batch_id = :bid")
        values[":bid"] = str(batch_id)
    if openai_batch_status is not None:
        expr_parts.append("openai_batch_status = :obs")
        values[":obs"] = str(openai_batch_status)
    if openai_batch_request_total is not None:
        expr_parts.append("openai_batch_request_total = :obrt")
        values[":obrt"] = int(openai_batch_request_total)
    if openai_batch_request_completed is not None:
        expr_parts.append("openai_batch_request_completed = :obrc")
        values[":obrc"] = int(openai_batch_request_completed)
    if openai_batch_request_failed is not None:
        expr_parts.append("openai_batch_request_failed = :obrf")
        values[":obrf"] = int(openai_batch_request_failed)
    if error_code is not None:
        expr_parts.append("error_code = :ec")
        values[":ec"] = str(error_code)
    if error_detail is not None:
        expr_parts.append("error_detail = :ed")
        values[":ed"] = str(error_detail)
...
def request_job_cancel(job_id: str, user_sub: str | None = None) -> tuple[str, int]:
    """
    Idempotent job cancel request.
    Returns (message, http_status) semantics for API layer:
    - ("ok", 200) enqueued cancel
    - ("not_found", 404)
    - ("forbidden", 403)
    - ("no_op", 200) already terminal
    """
    job = get_job(job_id)
    if not job:
        return "not_found", 404
    owner = job.get("user_sub")
    if user_sub and owner and owner != user_sub:
        return "forbidden", 403

    st = job.get("status", "")
    if st in ("completed", "failed", "cancelled"):
        return "no_op", 200
    if st == "cancelling" and job.get("cancel_requested_at"):
        return "no_op", 200
    ...
    try:
        table.update_item(
            Key={"job_id": job_id},
            UpdateExpression=(
                "SET cancel_requested_at = :c, updated_at = :u, #st = :s, "
                "phase = :ph, detail = :dt"
            ),
        ...
        if batch_id:
            try:
                from dmwe_core.llm.batch_client import cancel_batch

                cancel_batch(str(batch_id))
            except Exception as be:
                logger.warning("OpenAI batch cancel failed for %s: %s", batch_id, be)
        logger.info("Cancel requested for job %s", job_id)
        return "ok", 200
    except Exception as e:
        logger.error("request_job_cancel failed for %s: %s", job_id, str(e))
        return "error", 500
